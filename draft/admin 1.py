...
from classify_group import get_name_group
...
def admin():
    ...
    with col_left:
        with st.form('input_form'):
            ...
            # list_group = ["Select All"]
            folder_save_file_upload = os.path.join('data', st.session_state.code)
            # list_group.extend(get_name_group(folder_save_file_upload))
            list_group = ["Select All"] + get_name_group(folder_save_file_upload)
            list_group_copy = list(list_group)
            selected_options = col_left_prj_grid.multiselect("Dev", list_group_copy, placeholder="dev",
                                                             default=["Select All"])
            if 'Select All' in selected_options:
                selected_options = list_group[1:]
            st.write("selected_options: ", selected_options)
            ...
            if col_left_spec_grid.form_submit_button("View Data", use_container_width=True) and get_data(
                ...
                with st.spinner(text="In progress..."):
                    ...
                    session, data, project_id, app_list = query_data(str(st.session_state.code).upper(),
                                                                     ...
                                                                     st.session_state.case, "ALL", st.session_state.lot)
            ...
            if files is not None and col_left_spec_grid.form_submit_button("Load File",
                ...
                with st.spinner(text="In progress..."):
                    ...
                    list_group_1 = ["Select All"] + get_name_group(folder_save_file_upload)
                    st.session_state["Dev"] = list_group_1
                    set_data("running", 0)
            ...
            if col_left_spec_grid.form_submit_button("Create Cadics", use_container_width=True) == True and get_data(
                ...
                with st.spinner(text="In progress..."):
                    notice, session, data, project_id, app_list = create_cadics(st.session_state.case,
